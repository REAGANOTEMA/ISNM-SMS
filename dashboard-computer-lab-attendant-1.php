...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'staff' || $_SESSION['department'] !== 'computer-lab-attendant-1') {
...
$lab_info = [];
try {
    $stmt = $pdo->prepare("SELECT * FROM users WHERE id = ? AND role = 'staff' AND department = 'computer-lab-attendant-1'");
...
try {
    ...
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM computer_inventory WHERE lab_location = 'Lab 1'");
    $lab_stats['total_computers'] = $stmt->fetchColumn();
    
    // Available computers
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM computer_inventory WHERE lab_location = 'Lab 1' AND status = 'available'");
    $lab_stats['available_computers'] = $stmt->fetchColumn();
    
    // In use computers
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM computer_inventory WHERE lab_location = 'Lab 1' AND status = 'in-use'");
    $lab_stats['in_use_computers'] = $stmt->fetchColumn();
    
    // Under maintenance
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM computer_inventory WHERE lab_location = 'Lab 1' AND status = 'maintenance'");
    $lab_stats['maintenance_computers'] = $stmt->fetchColumn();
    
    // Today's lab sessions
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM lab_sessions WHERE lab_location = 'Lab 1' AND DATE(session_date) = CURDATE()");
    $lab_stats['today_sessions'] = $stmt->fetchColumn();
    
    // Active users
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM lab_usage WHERE lab_location = 'Lab 1' AND status = 'active'");
    $lab_stats['active_users'] = $stmt->fetchColumn();
    
    // Pending maintenance requests
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM maintenance_requests WHERE lab_location = 'Lab 1' AND status = 'pending'");
...
try {
    $stmt = $pdo->query("
        ...
        WHERE ls.lab_location = 'Lab 1' AND DATE(ls.session_date) = CURDATE()
...
try {
    $stmt = $pdo->query("
        ...
        WHERE mr.lab_location = 'Lab 1' AND mr.status = 'pending'
...
<head>
    ...
    <title>Computer Lab Attendant 1 Dashboard - ISNM</title>
...
<body>
    <nav class="navbar">
        <div class="nav-container">
            ...
            <div class="nav-links">
                <a href="dashboard-computer-lab-attendant-1.php" class="nav-link">
                ...
                <div class="user-menu">
                    ...
                    <span><?php echo htmlspecialchars($lab_info['username'] ?? 'Lab Attendant 1'); ?></span>
    ...
    <main class="main-container">
        <header class="dashboard-header fade-in">
            <div class="header-content">
                <h1 class="header-title">COMPUTER LAB ATTENDANT 1</h1>
