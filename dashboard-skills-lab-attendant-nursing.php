...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'staff' || $_SESSION['department'] !== 'skills-lab-attendant-nursing') {
...
$lab_info = [];
try {
    $stmt = $pdo->prepare("SELECT * FROM users WHERE id = ? AND role = 'staff' AND department = 'skills-lab-attendant-nursing'");
...
try {
    // Total equipment
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM skills_equipment WHERE lab_type = 'nursing'");
    $lab_stats['total_equipment'] = $stmt->fetchColumn();
    
    // Available equipment
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM skills_equipment WHERE lab_type = 'nursing' AND status = 'available'");
    $lab_stats['available_equipment'] = $stmt->fetchColumn();
    
    // In use equipment
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM skills_equipment WHERE lab_type = 'nursing' AND status = 'in-use'");
    $lab_stats['in_use_equipment'] = $stmt->fetchColumn();
    
    // Under maintenance
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM skills_equipment WHERE lab_type = 'nursing' AND status = 'maintenance'");
    $lab_stats['maintenance_equipment'] = $stmt->fetchColumn();
    
    // Today's lab sessions
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM skills_lab_sessions WHERE lab_type = 'nursing' AND DATE(session_date) = CURDATE()");
    $lab_stats['today_sessions'] = $stmt->fetchColumn();
    
    // Active students
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM skills_lab_usage WHERE lab_type = 'nursing' AND status = 'active'");
    $lab_stats['active_students'] = $stmt->fetchColumn();
    
    // Pending maintenance requests
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM equipment_maintenance WHERE lab_type = 'nursing' AND status = 'pending'");
...
try {
    $stmt = $pdo->query("
        ...
        WHERE sls.lab_type = 'nursing' AND DATE(sls.session_date) = CURDATE()
...
try {
    $stmt = $pdo->query("
        ...
        WHERE em.lab_type = 'nursing' AND em.status = 'pending'
...
<head>
    ...
    <title>Skills Lab Attendant (Nursing) Dashboard - ISNM</title>
...
<body>
    <nav class="navbar">
        <div class="nav-container">
            ...
            <div class="nav-links">
                <a href="dashboard-skills-lab-attendant-nursing.php" class="nav-link">
                ...
                <div class="user-menu">
                    ...
                    <span><?php echo htmlspecialchars($lab_info['username'] ?? 'Skills Lab Attendant (Nursing)'); ?></span>
    ...
    <main class="main-container">
        <header class="dashboard-header fade-in">
            <div class="header-content">
                <h1 class="header-title">SKILLS LAB ATTENDANT (NURSING)</h1>
                <p class="header-subtitle">Nursing Skills Lab Management & Training Excellence</p>
            </div>
        </header>
