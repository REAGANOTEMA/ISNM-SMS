...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'admin' || $_SESSION['department'] !== 'hr-manager') {
...
// Get HR manager information
$hr_info = [];
try {
    $stmt = $pdo->prepare("SELECT * FROM users WHERE id = ? AND role = 'admin' AND department = 'hr-manager'");
    $stmt->execute([$_SESSION['user_id']]);
    $hr_info = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log("Error fetching HR manager info: " . $e->getMessage());
}
...
try {
    ...
    $hr_stats['total_employees'] = $stmt->fetchColumn();
    
    // Active employees
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM users WHERE role != 'student' AND status = 'active'");
    $hr_stats['active_employees'] = $stmt->fetchColumn();
    
    // Pending leave requests
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM leave_requests WHERE status = 'pending'");
    $hr_stats['pending_leave'] = $stmt->fetchColumn();
    
    // Pending approvals
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM approvals WHERE status = 'pending'");
    $hr_stats['pending_approvals'] = $stmt->fetchColumn();
    
    // Open positions
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM job_postings WHERE status = 'open'");
    $hr_stats['open_positions'] = $stmt->fetchColumn();
    
    // Training sessions this month
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM training_sessions WHERE MONTH(created_at) = MONTH(CURRENT_DATE()) AND YEAR(created_at) = YEAR(CURRENT_DATE())");
    $hr_stats['training_this_month'] = $stmt->fetchColumn();
} catch (PDOException $e) {
...
try {
    $stmt = $pdo->query("
        SELECT lr.*, u.username as employee_name, d.department_name
        ...
        LIMIT 5
    ");
    $pending_leave = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
...
try {
    $stmt = $pdo->query("
        SELECT a.*, u.username as requester_name, d.department_name as department
        FROM approvals a
...
<body>
    <nav class="navbar">
        <div class="nav-container">
            ...
            <div class="nav-links">
                <a href="dashboard-hr-manager.php" class="nav-link">
                ...
                <div class="user-menu">
                    ...
                    <span><?php echo htmlspecialchars($hr_info['username'] ?? 'HR Manager'); ?></span>
    ...
    <main class="main-container">
        <header class="dashboard-header fade-in">
            <div class="header-content">
                <h1 class="header-title">HR MANAGER</h1>
                <p class="header-subtitle">Human Resources & Personnel Management Excellence</p>
            </div>
        </header>
        ...
        <div class="stats-grid fade-in">
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($hr_stats['total_employees'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($hr_stats['active_employees'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($hr_stats['pending_leave'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($hr_stats['pending_approvals'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($hr_stats['open_positions'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($hr_stats['training_this_month'] ?? 0); ?></div>
                <div class="stat-label">Training This Month</div>
            </div>
        </div>
                        ...
                        <div class="leave-item">
                            ...
                            <div class="leave-content">
                                <div class="leave-text">
                                    ...
                                    from <?php echo htmlspecialchars($leave['department_name'] ?? 'Unknown'); ?>
                                    requests <?php echo htmlspecialchars($leave['leave_type'] ?? 'Leave'); ?>
                                    for <?php echo htmlspecialchars($leave['days'] ?? 'N/A'); ?> days
                                </div>
                            ...
                            <div class="leave-status status-pending">
                                <?php echo htmlspecialchars(ucfirst($leave['status'] ?? 'Pending')); ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                        ...
                        <div class="leave-item">
                            ...
                            <div class="leave-content">
                                <div class="leave-text">
                                    ...
                                    from <?php echo htmlspecialchars($approval['department'] ?? 'Unknown'); ?>
        ...
        <div class="panel fade-in">
            <div class="panel-header">
                <h2 class="panel-title">
                    <i class="fas fa-tasks"></i> HR Manager Quick Actions
                </h2>
            </div>
    ...
    <script>
        ...
        const cards = document.querySelectorAll('.stat-card, .leave-item');
        cards.forEach(card => {
