...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'staff' || $_SESSION['department'] !== 'warden') {
...
$warden_info = [];
try {
    $stmt = $pdo->prepare("SELECT * FROM users WHERE id = ? AND role = 'staff' AND department = 'warden'");
    $stmt->execute([$_SESSION['user_id']]);
    $warden_info = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log("Error fetching Warden info: " . $e->getMessage());
}
...
try {
    // Total boys students
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM users WHERE role = 'student' AND gender = 'male'");
    $warden_stats['total_boys'] = $stmt->fetchColumn();
    
    // Boys in dormitories
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM student_dormitories WHERE gender = 'male'");
    $warden_stats['dormitory_boys'] = $stmt->fetchColumn();
    
    // Present today
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM attendance WHERE gender = 'male' AND DATE(attendance_date) = CURDATE() AND status = 'present'");
    $warden_stats['present_today'] = $stmt->fetchColumn();
    
    // Absent today
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM attendance WHERE gender = 'male' AND DATE(attendance_date) = CURDATE() AND status = 'absent'");
    $warden_stats['absent_today'] = $stmt->fetchColumn();
    
    // Medical cases this week
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM student_medical_cases WHERE gender = 'male' AND DATE(created_at) >= DATE_SUB(NOW(), INTERVAL 7 DAY)");
    $warden_stats['medical_cases_week'] = $stmt->fetchColumn();
    
    // Discipline cases this month
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM discipline_cases WHERE gender = 'male' AND MONTH(created_at) = MONTH(CURRENT_DATE())");
    $warden_stats['discipline_cases_month'] = $stmt->fetchColumn();
    
    // Pending leave requests
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM leave_requests WHERE gender = 'male' AND status = 'pending'");
    $warden_stats['pending_leave_requests'] = $stmt->fetchColumn();
} catch (PDOException $e) {
    error_log("Error fetching Warden statistics: " . $e->getMessage());
}
...
try {
    $stmt = $pdo->query("
        ...
        WHERE a.gender = 'male' AND DATE(a.attendance_date) = CURDATE() AND a.status = 'absent'
...
try {
    $stmt = $pdo->query("
        ...
        WHERE lr.gender = 'male' AND lr.status = 'pending'
...
<head>
    ...
    <title>Warden/Area Manager Dashboard - ISNM</title>
...
<body>
    <nav class="navbar">
        <div class="nav-container">
            ...
            <div class="nav-links">
                <a href="dashboard-warden.php" class="nav-link">
                ...
                <div class="user-menu">
                    ...
                    <span><?php echo htmlspecialchars($warden_info['username'] ?? 'Warden/Area Manager'); ?></span>
    ...
    <main class="main-container">
        <header class="dashboard-header fade-in">
            <div class="header-content">
                <h1 class="header-title">WARDEN/AREA MANAGER</h1>
                <p class="header-subtitle">Boys Student Welfare & Dormitory Management Excellence</p>
            </div>
        </header>
        ...
        <div class="stats-grid fade-in">
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($warden_stats['total_boys'] ?? 0); ?></div>
                <div class="stat-label">Total Boys Students</div>
            </div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($warden_stats['dormitory_boys'] ?? 0); ?></div>
                <div class="stat-label">Boys in Dormitories</div>
            </div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($warden_stats['present_today'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($warden_stats['absent_today'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($warden_stats['medical_cases_week'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($warden_stats['discipline_cases_month'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo number_format($warden_stats['pending_leave_requests'] ?? 0); ?></div>
        ...
        <div class="panel fade-in">
            <div class="panel-header">
                <h2 class="panel-title">
                    <i class="fas fa-tasks"></i> Warden Quick Actions
                </h2>
            </div>
